...
export default function PdfToDocx(){
  ...
  async function handleSubmit(e){
    ...
    try{
      ...
      const res = await fetch('/pdf-to-docx-convert', { method:'POST', body: fd })
      if (!res.ok){ const txt = await res.text(); throw new Error(txt || res.statusText) }
      const blob = await res.blob(); const filename = (file ? file.name.replace(/\.pdf$/i, '.docx') : 'converted.docx')
      ...
      <div className="content">
        <div className="upload-section">
          ...
          <form onSubmit={handleSubmit} className="upload-form">
            ...
            <br />
            <button type="submit" className="submit-btn" disabled={loading}>{loading? 'Converting...' : 'Convert to Word (DOCX)'}</button>
          </form>
        </div>
      </div>
    </div>
